...
- **自动切回直连**：掉线后立即把绑定在该出口上的入站解绑，流量回落到**机器本身的直连出口**，
  节点链接保持可用，不会出现"连得上但没速度"。
- **自动切回 SOCKS5**：出口隧道换节点重连成功后，入站自动重新绑回到 SOCKS5 出口，
  恢复原分流。
- **通知文案按实际解绑数区分**：只有出口**真的有入站绑定**被解绑时，掉线通知才会写
  "已自动切回直连"；如果该出口当前没有绑定任何入站（例如你在面板里把它切走了，
  或从来只有一条隧道被绑定），通知只提示"正在尝试换节点重连"，不会误导说流量切走了。
...
- **关闭掉线切直连**：不需要自动切换的话，面板里把入站绑到「直连」即可；
