...
- 🔁 **出口自动切换**：出口掉线自动切回直连保可用，恢复后自动切回 SOCKS5
- 📨 **Telegram 通知**：节点链接更新、出口掉线/恢复都会带机器名推送到你的 TG
...
fanout 的 VPNGate 出口（志愿者节点）运行中掉线很常见。**本版本已内置自动切换 + 通知**：

- **判掉线**：fanout 每 10 秒探测隧道出口是否还在 VPN 上，**连续 3 次失败（约 30 秒）
  才判定该出口掉线**。探测做了三重防抖：
  1. 同时打 3 个 HTTPS 出口 IP 接口（api.ipify.org / icanhazip / ifconfig.me），
     **任一成功即视为能出网**，单个接口抖动不算失败；
  2. 强制 IPv4 + HTTPS，避免明文 HTTP 被出口侧劫持、或 curl 走了 IPv6 导致
     探测结果与建隧道时对不上而误判；
  3. 出口 IP 与建隧道时不同**不算掉线**（VPNGate 节点常有多出口/负载均衡），
     只有当出口 IP **退回到母机自己的公网 IP**（说明 openvpn 确实断了、流量
     已经退回母机直连）或 3 个接口全部失败时，才真正判掉。
- **自动迁移绑定（优先）**：掉线后如果还有其他**已连通**的出口，绑定在该出口上的入站会
  **整体自动迁到备用出口**——分流不中断，面板显示的就是新出口，无需手动重绑；
  只有**没有其他可用出口**时，才把入站解绑回**机器本身的直连出口**（节点链接仍可用，
  不会出现"连得上但没速度"）。
- **自动迁回原出口**：出口隧道换节点重连成功后，之前被迁走的入站自动重新绑回这条
  SOCKS5 出口，恢复原分流，面板同步显示。
- **通知文案如实反映去向**：掉线通知会写明绑定的入站去了哪里——"已自动切到备用出口
  xxx"、"已自动切回直连（机器本身出口）"、或（该出口没有绑定任何入站时）
  "正在尝试换节点重连（该出口当前没有入站绑定，节点链接不受影响）"，不会误导说流量切走了。
- **Telegram 通知**（需要安装时启用了 TG 推送）：掉线推一条、恢复推一条，消息带**机器名**。
  恢复通知**无论该出口有没有入站绑定都会推**：有绑定的写"已切回 SOCKS5 出口"，
  没绑定的写"已恢复正常出网"——掉线 ⚠️ 之后必然有恢复 ✅，不会只有一条有去无回。
  掉线通知还会写清楚**确认过出口确实不可用**（连续 3 次探测失败，不是网络抖动），
  并把确认的结果（3 个接口全失败 / 出口 IP 退回母机）也带上。确认原因是**中文汇总**的
  （例如"全部 3 个探测接口失败：探测超时无响应…"，不会出现 signal: killed、
  context deadline exceeded 这类英文底层错误）。有备用出口时掉线的例子：

  ```
  ⚠️ [acck香港] fanout SOCKS5 出口掉线
  节点: vpn393802235
  槽位: 1
  本地端口: 29381
  已确认出口不可用：全部 3 个探测接口失败：探测超时无响应（8 秒内无数据返回，探测进程被强制结束）
  已自动切到备用出口 vpn123456789，正在尝试换节点重连

  ✅ [acck香港] fanout SOCKS5 出口已恢复
  ...
  已切回 SOCKS5 出口
  ```
...
- **机器名来源**：优先取环境变量 `FANOUT_NODE_NAME`，否则读 `~/.config/sb-argo/config.conf`
  的 `NODE_NAME`（安装时填的节点名称）。多台服务器时靠它区分是哪台发的。
- **手动切换不受影响**：面板里随时可手动切直连 / 切出口（「出口」下拉，改动即生效）；
  自动恢复只会绑回你当前选中的出口。
- **面板永远如实显示绑定**：节点详情里的「出口」下拉即使绑定出口暂时不在已连通状态
  （连接中/失败），也会把它列出并选中并标注状态，不会没头没脑地显示成"直连"。
- **通知去重**：每条隧道掉线只提醒一次，恢复后再掉线才会再提醒，不会刷屏。
- **关闭掉线自动切换**：不需要自动切换的话，面板里把入站绑到「直连」即可；
  或在面板后端模式里不绑定任何隧道，入站始终走直连。
- **多隧道同时上线不会互相覆盖配置**：fanout 写 sing-box.json 采用互斥锁 + 唯一临时文件名，
  多条隧道同一时刻启动时写入会串行化，不再出现并发重命名竞态导致配置更新丢失
  （旧版本偶发的 `rename ... no such file or directory`）。

> 说明：掉线探测与出口 IP 校验均在本机执行，无需额外配置；
> 若未启用 Telegram，则只自动切换、不推送通知。
