...
func main() {
	...
	panelMode := flag.String("panel", "", "节点链接后端: 留空按界面设置/自动探测, 3x-ui, native, xray-cf-lite, sing-box-argo-lite")
...
func apiXUIStatus(w http.ResponseWriter, r *http.Request) {
	...
	// xray-cf-lite / sing-box-argo-lite 模式下节点由对应工具管，fanout 只改路由，不提供新建入口。
	canCreate := true
	switch p.(type) {
	case *XCL, *SBA:
		canCreate = false
