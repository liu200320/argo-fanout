...
<div class="modal" id="settings">
  <div class="sheet">
    ...
    <div class="body">
      ...
      <div class="hint" id="setBackendHint">节点从哪来。装了 3x-ui、xray-cf-lite 或 sing-box-argo-lite 就能直接接管，都没有就用自建。</div>
...
// 接管型后端（xray-cf-lite / sing-box-argo-lite）的节点归它们自己管，
// fanout 只改路由，界面不给新建/删除/改节点的入口
function isXCL(){ return view.backend === 'xray-cf-lite' || view.backend === 'sing-box-argo-lite'; }
const BACKEND_NAME = {'native':'自建 Xray', '3x-ui':'3x-ui', 'xray-cf-lite':'xray-cf-lite', 'sing-box-argo-lite':'sing-box-argo-lite'};
...
async function poll(){
  try{
    ...
    $('#panel').textContent = view.panel
      ...
      : (view.panel_info || '');
  // 接管型后端的节点由它自己生成，fanout 这边只管把它们导到哪条出口
    $('#newnode').hidden = isXCL();
    // 链接由对应工具的订阅体系发，fanout 这边导不出来
...
async function loadWizard(){
  ...
  const sel = $('#tpl');
  // 接管型后端不能复制节点，向导退化成"只开出口"，之后在节点详情里挑出口
  if(isXCL()){
    $('#tplwrap').hidden = true;
...
function renderDetail(d){
  ...
  $('#dbody').innerHTML = '<dl class="kv">'
    ...
    : '<div class="hint">这个节点由 ' + esc(backendName()) + ' 管，端口、UUID 和分享链接都去它那边改。这里只决定它走哪条出口。</div>');
}
...
async function loadBackendModes(){
  ...
  try{
    ...
    hint.textContent = m.describe
      ...
      : '节点从哪来。装了 3x-ui、xray-cf-lite 或 sing-box-argo-lite 就能直接接管，都没有就用自建。';
  }catch(err){
